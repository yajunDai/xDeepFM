...
class arg():
    def __init__(self,FLAGS):
        # data
        self.train_file = FLAGS['train_file'] if 'train_file' in FLAGS else None
        self.eval_file = FLAGS['eval_file'] if 'eval_file' in FLAGS else None
        self.test_file = FLAGS['test_file'] if 'test_file' in FLAGS else None
        self.infer_file = FLAGS['infer_file'] if 'infer_file' in FLAGS else None
        self.FEATURE_COUNT = FLAGS['FEATURE_COUNT'] if 'FEATURE_COUNT' in FLAGS else None
        self.FIELD_COUNT = FLAGS['FIELD_COUNT'] if 'FIELD_COUNT' in FLAGS else None
        self.data_format = FLAGS['data_format'] if 'data_format' in FLAGS else None
        self.PAIR_NUM = FLAGS['PAIR_NUM'] if 'PAIR_NUM' in FLAGS else None
        self.DNN_FIELD_NUM = FLAGS['DNN_FIELD_NUM'] if 'DNN_FIELD_NUM' in FLAGS else None
        self.n_user = FLAGS['n_user'] if 'n_user' in FLAGS else None
        self.n_item = FLAGS['n_item'] if 'n_item' in FLAGS else None
        self.n_user_attr = FLAGS['n_user_attr'] if 'n_user_attr' in FLAGS else None
        self.n_item_attr = FLAGS['n_item_attr'] if 'n_item_attr' in FLAGS else None
        # model
        self.dim = FLAGS['dim'] if 'dim' in FLAGS else None
        self.layer_sizes = FLAGS['layer_sizes'] if 'layer_sizes' in FLAGS else None
        self.cross_layer_sizes = FLAGS['cross_layer_sizes'] if 'cross_layer_sizes' in FLAGS else None
        self.cross_layers = FLAGS['cross_layers'] if 'cross_layers' in FLAGS else None
        self.activation = FLAGS['activation'] if 'activation' in FLAGS else None
        self.cross_activation = FLAGS['cross_activation'] if 'cross_activation' in FLAGS else "identity"
        self.dropout = FLAGS['dropout'] if 'dropout' in FLAGS else None
        self.attention_layer_sizes = FLAGS['attention_layer_sizes'] if 'attention_layer_sizes' in FLAGS else None
        self.attention_activation = FLAGS['attention_activation'] if 'attention_activation' in FLAGS else None
        self.model_type = FLAGS['model_type'] if 'model_type' in FLAGS else None
        self.method = FLAGS['method'] if 'method' in FLAGS else None
        self.load_model_name = FLAGS['load_model_name'] if 'load_model_name' in FLAGS else None
        self.mu = FLAGS['mu'] if 'mu' in FLAGS else None
        # train
        self.init_method = FLAGS['init_method'] if 'init_method' in FLAGS else 'tnormal'
        self.init_value = FLAGS['init_value'] if 'init_value' in FLAGS else 0.01
        self.embed_l2 = FLAGS['embed_l2'] if 'embed_l2' in FLAGS else 0.0000
        self.embed_l1 = FLAGS['embed_l1'] if 'embed_l1' in FLAGS else 0.0000
        self.layer_l2 = FLAGS['layer_l2'] if 'layer_l2' in FLAGS else 0.0000
        self.layer_l1 = FLAGS['layer_l1'] if 'layer_l1' in FLAGS else 0.0000
        self.cross_l2 = FLAGS['cross_l2'] if 'cross_l2' in FLAGS else 0.0000
        self.cross_l1 = FLAGS['cross_l1'] if 'cross_l1' in FLAGS else 0.0000
        self.learning_rate = FLAGS['learning_rate'] if 'learning_rate' in FLAGS else 0.001
        self.loss = FLAGS['loss'] if 'loss' in FLAGS else None
        self.optimizer = FLAGS['optimizer'] if 'optimizer' in FLAGS else 'adam'
        self.epochs = FLAGS['epochs'] if 'epochs' in FLAGS else 10
        self.batch_size = FLAGS['batch_size'] if 'batch_size' in FLAGS else 1
        # show info
        self.log = FLAGS['log'] if 'log' in FLAGS else "log"
        self.logger = None
        self.show_step = FLAGS['show_step'] if 'show_step' in FLAGS else 1
        self.save_epoch = FLAGS['save_epoch'] if 'save_epoch' in FLAGS else 5
        self.metrics = FLAGS['metrics'] if 'metrics' in FLAGS else None
...
def main():
    ...
    config = load_yaml() #dict,配置的字典
    check_config(config) # 检查配置，设置是否符合要求
    #print(config,type(config),config['data'])
    hparams = create_hparams(config) #将四部分flat
    log = Log(hparams) #记录日志
